...
with tab2:
    ...
    with st.sidebar:
        ...
        uploaded_file_tab2 = st.file_uploader("Sube tu archivo Excel (.xlsx)", 
                                            type=["xlsx"], 
                                            help="Archivo debe contener columnas: Turno, Doc Pac, Paciente, Sede, Sala Espera, Servicio, Llamados, Tipo, Hora Llegada, Hora Atención, Hora Finalización, Tiempo Espera, Tiempo Atención, Modulo Atención, Usuario Atención, Especialidad, ID Usuario, Estado",
                                            key="tab2_file")
    ...
    if uploaded_file_tab2 is not None:
        try:
            ...
            # --- PROCESAMIENTO DE FECHAS ---
            # Convertimos la columna HORA LLEGADA a datetime para poder operar
            df_tab2["HORA LLEGADA"] = pd.to_datetime(df_tab2["HORA LLEGADA"], errors='coerce')
            
            # Eliminamos filas con fechas nulas para evitar errores en el selector
            df_tab2 = df_tab2.dropna(subset=["HORA LLEGADA"])

            # Identificamos los límites reales del archivo
            fecha_minima_archivo_tab2 = df_tab2["HORA LLEGADA"].min().date()
            fecha_maxima_archivo_tab2 = df_tab2["HORA LLEGADA"].max().date()
            ...
            with st.sidebar:
                ...
                # 4. Filtro de Usuario Atención (equivalente a USUARIO CREA INGRESO)
                usuarios_tab2 = sorted(df_tab2["Usuario Atención"].dropna().unique())
            ...
            # --- APLICACIÓN DE FILTROS ---
            df_filtrado_tab2 = df_tab2.copy()

            # Filtrado por Rango de Fechas (solo si las fechas son válidas)
            if fecha_inicio_tab2 <= fecha_fin_tab2:
                df_filtrado_tab2 = df_filtrado_tab2[
                    (df_filtrado_tab2["HORA LLEGADA"].dt.date >= fecha_inicio_tab2) & 
                    (df_filtrado_tab2["HORA LLEGADA"].dt.date <= fecha_fin_tab2)
                ]
            ...
            # Filtrado por Usuario
            if usuario_sel_tab2:
                df_filtrado_tab2 = df_filtrado_tab2[df_filtrado_tab2["Usuario Atención"].isin(usuario_sel_tab2)]
            ...
            if procesar_tab2 and not df_filtrado_tab2.empty and fecha_inicio_tab2 <= fecha_fin_tab2:
                ...
                **Configuración de análisis:**
                ...
                # Extraer información de fecha y hora (usando HORA LLEGADA en lugar de FECHA CREACION)
                df_proceso_tab2['FECHA'] = df_proceso_tab2['HORA LLEGADA'].dt.date
                df_proceso_tab2['HORA'] = df_proceso_tab2['HORA LLEGADA'].dt.hour
                df_proceso_tab2['DIA_SEMANA'] = df_proceso_tab2['HORA LLEGADA'].dt.day_name()
                df_proceso_tab2['DIA_SEMANA_NUM'] = df_proceso_tab2['HORA LLEGADA'].dt.dayofweek  # 0=Lunes, 6=Domingo
                ...
                if df_proceso_tab2.empty:
                    ...
                    # Obtener lista de usuarios únicos (Usuario Atención)
                    usuarios_proceso_tab2 = sorted(df_proceso_tab2["Usuario Atención"].dropna().unique())
                    ...
                    if not usuarios_proceso_tab2:
                        ...
                        for usuario in usuarios_proceso_tab2:
                            df_usuario = df_proceso_tab2[df_proceso_tab2["Usuario Atención"] == usuario]
            ...
            import traceback
            st.code(traceback.format_exc())
            st.info("Verifica que el archivo tenga las columnas necesarias: 'HORA LLEGADA', 'Servicio', 'Usuario Atención'")
    else:
        st.info("👆 Usa la barra lateral para subir un archivo Excel y activar los filtros.")
        st.caption("El archivo debe contener al menos las columnas: 'HORA LLEGADA', 'Servicio', 'Usuario Atención'")
