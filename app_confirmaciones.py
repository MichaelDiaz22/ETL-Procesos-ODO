...
if uploaded_file is not None:
    ...
    fijo_is_valid_fallback = (~df['Telefono Fijo'].isna()) | (df['Telefono Fijo'] != '') | (df['Telefono Fijo'] != 'nan') | (~df['Telefono Fijo'].str.startswith('60', na=False)) # Added check for string 'nan' - Changed & to | based on previous logic
    ...
    if st.button("Generate and Download Files"):
        ...
        for i, file_filters in enumerate(filters):
            ...
            if file_filters['empresas']:
                # Case-insensitive comparison for Empresa filter
            ...
            if file_filters['ubicaciones']:
                # Case-insensitive comparison for Ubicación filter
